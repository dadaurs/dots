...
essid=$(ifconfig wlp2s0 | sed -n "1p" | awk -F '"' '{print $2}')
mode=$(ifconfig wlp2s0 | sed -n "1p" | awk -F " " '{print $3}')
freq=$(ifconfig wlp2s0 | sed -n "2p" | awk -F " " '{print $2}' | cut -d":" -f2)
mac=$(ifconfig wlp2s0 | sed -n "2p" | awk -F " " '{print $6}')
qual=$(ifconfig wlp2s0 | sed -n "6p" | awk -F " " '{print $2}' | cut -d"=" -f2)
lvl=$(ifconfig wlp2s0 | sed -n "6p" | awk -F " " '{print $4}' | cut -d"=" -f2)
rate=$(ifconfig wlp2s0 | sed -n "3p" | awk -F "=" '{print $2}' | cut -d" " -f1)
