...
echo "$1" | sed --posix -e 's/:/\ /g'\
	...
	-e "s/\ \ //g"\
	-e "s/1/%{A:bspc desktop -f 1:}1%{A}"
	-e "s/2/%{A:bspc desktop -f 2:}2%{A}"
	-e "s/3/%{A:bspc desktop -f 3:}3%{A}"
	-e "s/4/%{A:bspc desktop -f 4:}4%{A}"
	-e "s/5/%{A:bspc desktop -f 5:}5%{A}"
	-e "s/6/%{A:bspc desktop -f 6:}6%{A}"
	-e "s/7/%{A:bspc desktop -f 7:}7%{A}"
	-e "s/8/%{A:bspc desktop -f 8:}8%{A}"
	-e "s/9/%{A:bspc desktop -f 9:}9%{A}"
	-e "s/10/%{A:bspc desktop -f 1:}1%{A}"
